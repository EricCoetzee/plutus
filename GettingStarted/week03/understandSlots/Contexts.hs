...
data ScriptPurpose
    = Minting CurrencySymbol -- this is used for defining Native Tokens :: To describe under which circumstances the native tokens may be minted
    | Spending TxOutRef -- this is the most imports purpose of the ScriptContext
    | Rewarding StakingCredential -- Used for staking
    | Certifying DCert -- Used for Certifying Certificates(Deligating certificates)

-- | A pending transaction. This is the view as seen by validator scripts, so some details are stripped out.
data TxInfo = TxInfo --Describes the spending transaction
    ...
    , txInfoValidRange  :: SlotRange -- ^ The valid range for the transaction. :: Time interval -> is time falls within this slot range transation will succeeds else it will rejected without running the validationScripts
    , txInfoSignatories :: [PubKeyHash] -- ^ Signatures provided with the transaction, attested that they all signed the tx
    , txInfoData        :: [(DatumHash, Datum)]
    , txInfoId          :: TxId -- Transaction ID that contains all it's inputs and outputs
    -- ^ Hash of the pending transaction (excluding witnesses)
    } deriving (Generic)

data ScriptContext = ScriptContext{scriptContextTxInfo :: TxInfo, scriptContextPurpose :: ScriptPurpose }
-- ScriptContext is a record type with 2 fields 
-- The actual context is defined in the 'scriptContextTxInfo'
